...
public class ThreadingDictionary<TK, TV> : ConcurrentDictionary<TK, TV>
{
    /// <summary>
    ...
    public virtual void Add(TK key, TV value, bool replace = false)
    {
        ...
        if (!ContainsKey(key))
            throw new AccessViolationException($"No access for adding {nameof(key)} to the ConcurrentDictionary.");
    }
    ...
    public virtual void RemoveKey(TK key)
    {
        ...
        if (ContainsKey(key))
            throw new AccessViolationException($"No access for removing {nameof(key)} from the ConcurrentDictionary.");
    }
    ...
    public virtual TV GetValue(TK key)
    {
        ...
        if (!accessGranted)
            throw new AccessViolationException(
                $"No access for getting value {key?.ToString() ?? nameof(key)} from the ConcurrentDictionary.");
